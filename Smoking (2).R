...
smoking<-read.csv("C:/Users/utente/Desktop/università/data mining/smoking.csv")
str(smoking)
...
ggplot(subset(df,df$fasting.blood.sugar<300), aes(x=HDL,
                                                  fill=as.factor(smoking)))+
  geom_bar(alpha=0.8)+
  labs(title="HDL and Smoking", fill="smoking")+
  theme(legend.position = "bottom", plot.title =
          element_text(face="bold", hjust=0.5), text=element_text(size=18))+
...
# percui utilizzo la funzione symbox per vedere quale trasformazione ? pi? adatta
...
covEllipses(dati_smaller_trn[,nf], 
            ...
            variables = c(1:16, 16), 
            fill.alpha = 0.05)
#si nota che anche la covarianza ? comune tra le classi

#VERIFICO NORMALITA'
library(nortest)
ad.test(trn_0$relaxation)$p.value
ad.test(trn_1$relaxation)$p.value
# Costruzione dei qqplot per la verifica della normalit? delle osservazioni condizionatamente alla classe.
# Classe: 1
...
# Costruzione curve di densit? condizionate rispetto alla classe
dati_smaller_trn$smoking<-as.factor(dati_smaller_trn$smoking)
...
#solo poche delle variabili presentano una distribuzione normale percui non applico QDA e LDA
...
#ricalcolo il modello senza putni influenti e vedo se l'AIC migliora

model_glm2= glm(smoking ~. , data = trn_nuovo, 
                family = "binomial")
summary(model_glm2) #AIC 30750
step.model2 <- stepAIC(model_glm2, direction = "both", trace = FALSE)
summary(step.model2) #AIC 30741
...
#STANDARDIZZAZIONE utilizzando le medie e varianze del training
nf<-c(2:6,9:16,18:20)
for(t in nf){ #dati normalizzati
...
#----
#regressione logistica, con modello stimato sul training
...
step.modelc <- stepAIC(model_glmc, direction = "both", trace = FALSE) 
summary(step.modelc)
# aic 41226
...
#ricalcolo il modello senza punti influenti e vedo se il mio AIC migliora
...
#STANDARDIZZAZIONE con medie e varianze del trining completo 
nf<-c(2:6,9:16,18:20)
for(t in nf){ #dati normalizzati
...
#----
#regressione logistica, applico il modello stimao nel training completo 
...
acc_log_tst #accuracy 0.7524

pred_roclogit <- prediction(pred_logit, dati_tst[, 23])
perf_logit <- performance(pred_roclogit,"tpr","fpr")
auc_logit <- performance(pred_roclogit, measure = "auc")@y.values
auc_logit #AUC 0.8355
...
pred_logit_class <- ifelse (pred_logit > 0.5, 1, 0) 
dati_tst2[11140,21]<-"1"
dati_tst2[11140,1]<-"F"
dati_tst2[11140,8]<-"1"
dati_tst2[11140,7]<-"1"
dati_tst2[11140,17]<-"1"
...
#stimo il modello sul training completo 
training$smoking<-as.factor(training$smoking)
rf <- randomForest(smoking~., data=training,importance=TRUE, ntree=500)

#valori previsti sul training completo 
p1 <- predict(rf, training)
confusionMatrix(p1, training$smoking) #accuracy 1
#valori previsti sul test
dati_tst$smoking<-as.factor(dati_tst$smoking)
p2<-predict(rf, dati_tst)
confusionMatrix(p2, dati_tst$smoking) #accuracy 83%
...
ggplot(df_imp, aes(x=reorder(variable,-importance),y=importance,
                   fill=importance))+
  geom_col()+
  labs(x="Predictors", y ="Importance", title="Importance of predictor
variables")+
